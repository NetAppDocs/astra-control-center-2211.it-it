...
* Se intendi utilizzare più spazi dei nomi con le tue applicazioni in Astra Control, link:../use/manage-roles.html#add-a-namespace-constraint-to-a-role["modificare i ruoli utente con vincoli dello spazio dei nomi"] Dopo l'aggiornamento a una versione di Astra Control Center con supporto di più namespace.
